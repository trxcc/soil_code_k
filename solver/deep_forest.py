...
class DeepForestModel(RegressionModel):
    def __init__(
        self,
        name: str = "DeepForest",
        task_name: str = "",
        ...
        if task_name == "MurA":
            self.max_depth = 14
            self.n_estimators = 13
            self.min_samples_split = 4
        elif task_name == "GlcN":
            self.max_depth = 14
            self.n_estimators = 91
            self.min_samples_split = 2
        elif task_name == "F_GN":
            self.max_depth = 20
            self.n_estimators = 11
            self.min_samples_split = 2
        elif task_name == "A_GN":
            self.max_depth = 18
            self.n_estimators = 144
            self.min_samples_split = 2
        elif task_name == "k":
            self.max_depth = 15
            ...
            self.max_depth = 13
            self.n_estimators = 154
            self.min_samples_split = 2
        elif task_name == "k-241004":
            self.max_depth = 17
            self.n_estimators = 138
            self.min_samples_split = 3
    ...
    def _optimize_hyperparameters_optuna(
        ...
        def object_func(trial):
            ...
            model = CascadeForestRegressor(
                random_state=self.random_state, n_jobs=-1, **trial_args_dict
            )
    ...
    def optimize_hyperparameters(
        ...
        if optimize_method.lower() == "bayesopt":
            ...
            return {"max_depth": 17, "n_estimators": 18, "min_samples_split": 10}
    ...
    def _optimize_hyperparameters_grid_search(
        ...
        model = CascadeForestRegressor(random_state=self.random_state, n_jobs=-1)
    ...
    def save(self, save_path) -> None:
        ...
        with open(os.path.join(save_path, "DeepForestModel.pkl"), "wb") as f:
            pickle.dump(self.model, f)

    def load(self, model_dir):
        model_path = os.path.join(model_dir, "DeepForestModel.pkl")
