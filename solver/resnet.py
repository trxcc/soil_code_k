...
class ResNetModel(RegressionModel):
    def __init__(
        self,
        name: str = "ResNet",
        task_name: str = "",
    ...
    def fit(
        ...
        for epoch in range(n_epochs):
            ...
            for batch in tqdm(
                delu.iter_batches(
                    self.data["train"], batch_size, shuffle=True, drop_last=False
                ),
                desc=f"Epoch {epoch}",
                total=epoch_size,
            ):
    ...
    def optimize_hyperparameters(
        ...
        def object_func(trial):
            ...
            self.model = ResNet(
                ...
                d_hidden=None,
                d_hidden_multiplier=2.0,
                dropout1=trial_args_dict["dropout1"],
                dropout2=0.0,
            ).to(self.device)
    ...
    def save(self, save_path=None) -> None:
        ...
        torch.save(checkpoint, os.path.join(save_path, "ResNet.pt"))
    ...
    def load(self, model_dir, preprocessing, y_mean, y_std):
        ...
        self.Y_mean = y_mean
        self.Y_std = y_std
        model_path = os.path.join(model_dir, "ResNet.pt")
